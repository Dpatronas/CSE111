...
printf "\n%30s:  %2d/%2d   %5.1f%% of 50%%\n" "Functional" $pass $tests $total

spass=`grep 'Speed for ' perf.out | grep PASS | wc -l`
sfail=`grep 'Speed for ' perf.out | grep FAIL | wc -l`
...
printf "%30s:  %2d/%2d   %5.1f%% of 40%%\n" "Non Functional" $pass $tests $ptotal
