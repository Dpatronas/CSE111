    ...
    if (htonl(DG.flag) == LAST) { // Last DG
      ...
      for (auto &elem : list) {
        ...
        if (DG.num_values == MAX_VALUES) {
          ...
          n = sendto(sockid, (void *) &DG, sizeof(message_t), 0, (struct sockaddr *)&remote_addr, len);
          if (n < 0) exit(-1);
...
//==================================================================================
  ...
  for (std::vector<unsigned int> &list : lists) {
    unsigned int seq = 0; // holds max number of Datagrams-1
    ...
    for (unsigned int i = 0; i < num_DG; i++) {
      ...
      // Send DG
      n = sendto(sockfd, (void *)& DG, sizeof(message_t), 0, (struct sockaddr *)&remote_addr, len);
      if (n < 0) exit(-1);
    }
  ...
  ///////////////////////// RECEIVE DATAGRAMS FROM SERVER  ////////////////////////////////
    ...
    for (;;) {
      ...
      if (tryReceive == 0) {
        ...
        n = sendto(sockfd, (void *) &on_wire, sizeof(message_t), 0, (struct sockaddr *)&remote_addr, len);
        continue;
      }

      n = recvfrom(sockfd, (void*)&on_wire, sizeof(message_t), 0, (struct sockaddr *)&remote_addr, &len);
      if (n < 0) exit (-1);
